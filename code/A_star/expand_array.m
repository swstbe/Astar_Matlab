function expand_array=expand_array(node_x,node_y,gn,xTarget,yTarget,CLOSED,MAX_X,MAX_Y)
        ...
        for j = 1:-1:-1
            if (k~=0 || j~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))%node within array bound
                    ...
                    if (flag == 1)
                        expand_array(expand_count,1) = s_x;
                        expand_array(expand_count,2) = s_y;
                        expand_array(expand_count,3) = distance(xTarget,yTarget,s_x,s_y);%distance between node and goal,hn
                        expand_array(expand_count,4) = gn+distance(node_x,node_y,s_x,s_y);%cost of travelling to node£¬gn
                        expand_array(expand_count,5) = expand_array(expand_count,3)+expand_array(expand_count,4);%fn
